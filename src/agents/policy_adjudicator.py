...
def run_policy_adjudicator(runtime: AgentRuntime, case: dict[str, Any], dossier: dict[str, Any], defects: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    system = f"""You are the Policy Adjudicator Agent. You have no raw-data tools. Read the authoritative policy and specialist reports. Reason over reports, explicitly checking primary conditions in priority order before selecting an outcome. Never invent facts or evidence. Return JSON only. Include a complete `final_output` candidate matching the repository output schema, plus structured justification.\n\n{POLICY_TEXT}"""
